...
We need to declare the two additional parameter for the oxygen exchange velocity and the constant consumption in the sediment:
...
And we need to declare the two functions used $log$, and $exp$, as well as the external dependencies $p$ (the barometric pressure at the surface), and $Temp$ (water temperature) which are needed to calculate the oxygen saturation concentration:
...
Create fortran source code, compile *GOTM-FABM*, run the model, and plot results:
...
if wanted `rodeoFABM` can automatically generate LaTeX documentation of the state variables, parameter, processes and stoichometry. Still under developement! For table generation use e.g. the package `xtable`.
